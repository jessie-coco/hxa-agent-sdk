 ...
 */
export function parseMessageContent(message: IncomingMessage): ParsedContent {
 ...
 */
export function resolveMediaUrl(urlOrPath: string, baseUrl: string): string {
...
function parseText(raw: unknown): TextContent {
  if (typeof raw === 'string') return { type: 'text', text: raw }
  const o = obj(raw)
  return { type: 'text', text: str(o.text ?? o.content ?? '') }
}

function parseImage(raw: unknown): ImageContent {
...
function parseFile(raw: unknown): FileContent {
  if (typeof raw === 'string') return { type: 'file', url: raw, fileName: 'file' }
  const o = obj(raw)
  return {
    type: 'file',
    url: str(o.url),
...
function parseVoice(raw: unknown): VoiceContent {
  if (typeof raw === 'string') return { type: 'voice', url: raw }
  const o = obj(raw)
  return { type: 'voice', url: str(o.url), duration: num(o.duration) }
}

function parseVideo(raw: unknown): VideoContent {
  if (typeof raw === 'string') return { type: 'video', url: raw }
  const o = obj(raw)
  return {
    type: 'video',
    url: str(o.url),
...
function parseCard(raw: unknown): CardContent {
  const o = obj(raw)
  return {
    type: 'card',
    title: str(o.title ?? 'Untitled'),
    subtitle: o.subtitle ? str(o.subtitle) : undefined,
    fields: Array.isArray(o.fields) ? o.fields.map((f: unknown) => {
      const fo = obj(f)
      return { label: str(fo.label), value: str(fo.value) }
    }) : undefined,
    actions: Array.isArray(o.actions) ? o.actions.map((a: unknown) => {
      const ao = obj(a)
...
function parseLocation(raw: unknown): LocationContent {
  const o = obj(raw)
  return {
    type: 'location',
...
function parseEmojiBig(raw: unknown): EmojiBigContent {
  if (typeof raw === 'string') return { type: 'emoji_big', emoji: raw }
  const o = obj(raw)
  return { type: 'emoji_big', emoji: str(o.emoji ?? o.text ?? o.content ?? '') }
}

function parsePoll(raw: unknown): PollContent {
  const o = obj(raw)
  return {
    type: 'poll',
...
function parseBurnAfterReading(raw: unknown): BurnAfterReadingContent {
  if (typeof raw === 'string') return { type: 'burn_after_reading', text: raw }
  const o = obj(raw)
